...
@patch('src.jobs.execute.place_sell_limit_order', new_callable=AsyncMock)
@patch('src.jobs.track.KalshiClient')
async def test_non_resolution_exit_pending_close_after_successful_sell(
    mock_kalshi_client, mock_place_sell
):
    """
    When a non-resolution sell order is placed successfully, the position
    should be marked as 'pending_close' (not 'closed') because the limit
    order may not have filled yet. No trade log should be written until
    fill is confirmed.
    """
    ...
    try:
        ...
        assert position_after is not None
        assert position_after.status == "pending_close", (
            "Position should be pending_close until sell order fill is confirmed"
        )

        trade_logs = await db_manager.get_all_trade_logs()
        assert len(trade_logs) == 0, (
            "No trade log should be written until sell order fill is confirmed"
        )
    finally:
        if os.path.exists(db_path):
            os.remove(db_path)
...
@patch('src.jobs.track.KalshiClient')
async def test_resolution_pnl_yes_win(mock_kalshi_client):
    """
    Test that P&L uses binary settlement for resolved markets:
    YES position wins -> P&L = (1.0 - entry_price) * quantity
    """
    db_path = "track_test_pnl_yes_win.db"
    if os.path.exists(db_path):
        os.remove(db_path)
    ...
    test_position = Position(
        market_id="PNL-YES-WIN",
        side="YES",
        entry_price=0.40,
        quantity=10,
        timestamp=datetime.now(),
        rationale="YES position that wins",
        confidence=0.80,
        live=True,
        status="open",
    )
    ...
    try:
        ...
        # Binary settlement: win = (1.0 - 0.40) * 10 = 6.0
        assert trade_logs[0].pnl == pytest.approx(6.0)
    finally:
        if os.path.exists(db_path):
            os.remove(db_path)
...
@patch('src.jobs.track.KalshiClient')
async def test_resolution_pnl_no_win(mock_kalshi_client):
    """
    Test that P&L uses binary settlement for NO-side positions:
    NO position wins (result=NO) -> P&L = (1.0 - entry_price) * quantity
    """
    db_path = "track_test_pnl_no_win.db"
    if os.path.exists(db_path):
        os.remove(db_path)
    ...
    test_position = Position(
        market_id="PNL-NO-WIN",
        side="NO",
        entry_price=0.35,
        quantity=8,
        timestamp=datetime.now(),
        rationale="NO position that wins",
        confidence=0.70,
        live=True,
        status="open",
    )
    ...
    try:
        ...
        trade_logs = await db_manager.get_all_trade_logs()
        assert len(trade_logs) == 1
        # Binary settlement: NO wins = (1.0 - 0.35) * 8 = 5.20
        assert trade_logs[0].pnl == pytest.approx(5.20)
    finally:
        if os.path.exists(db_path):
            os.remove(db_path)
...
@patch('src.jobs.track.KalshiClient')
async def test_resolution_pnl_no_lose(mock_kalshi_client):
    """
    Test that P&L uses binary settlement for NO-side losses:
    NO position loses (result=YES) -> P&L = -entry_price * quantity
    """
    db_path = "track_test_pnl_no_lose.db"
    if os.path.exists(db_path):
        os.remove(db_path)
    ...
    test_position = Position(
        market_id="PNL-NO-LOSE",
        side="NO",
        entry_price=0.35,
        quantity=8,
        timestamp=datetime.now(),
        rationale="NO position that loses",
        confidence=0.70,
        live=True,
        status="open",
    )
    ...
    try:
        ...
        trade_logs = await db_manager.get_all_trade_logs()
        assert len(trade_logs) == 1
        # Binary settlement: NO loses = -0.35 * 8 = -2.80
        assert trade_logs[0].pnl == pytest.approx(-2.80)
    finally:
        if os.path.exists(db_path):
            os.remove(db_path)
