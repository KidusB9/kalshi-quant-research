...
async def _evaluate_immediate_trade(
    ...
    try:
        ...
        if not can_add_position:
            ...
            logger.info(f" Position size ${initial_position_size:.2f} exceeds limits, attempting to reduce...")
        ...
        if position_count > 0:
            logger.info(f" Skipping immediate trade for {opportunity.market_id} - position already exists")
            return
        ...
        if min_cost > available_cash:
            logger.info(f" Cannot afford minimum 1 contract: ${min_cost:.2f} > ${available_cash:.2f}")
            return
        ...
        try:
            ...
            if market_status not in ['active', 'open']:
                logger.warning(f" Skipping {opportunity.market_id} - Market status: {market_status} (not active/open)")
                return
            
            if not (yes_ask and no_ask and yes_ask > 0 and no_ask > 0):
                logger.warning(f" Skipping {opportunity.market_id} - No valid prices (YES={yes_ask:.4f}, NO={no_ask:.4f})")
                return
            ...
            if yes_ask >= 0.99 and no_ask >= 0.99:
                logger.warning(f" Skipping {opportunity.market_id} - Collection/series ticker (YES={yes_ask:.4f}, NO={no_ask:.4f})")
                return
                
            logger.info(f" Market validation passed for {opportunity.market_id} - Status: {market_status}, proceeding with trade!")
            
        except Exception as e:
            logger.error(f" Skipping {opportunity.market_id} - Market validation failed: {e}")
            import traceback
