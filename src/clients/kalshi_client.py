...
class KalshiClient(TradingLoggerMixin):
    ...
    async def get_events(
        ...
        """
        Get events, optionally with their nested markets.

        Events carry the ``mutually_exclusive`` flag that identifies a true
        partition of outcomes -- the only reliable basis for complete-set
        arbitrage detection (market_id string-grouping is unreliable).

        Args:
            limit: Maximum number of events to return (max 200).
            cursor: Pagination cursor.
            status: Filter by status (e.g. "open").
            with_nested_markets: Include each event's markets inline.

        Returns:
            Events data (each event includes ``markets`` when nested).
        """
    ...
    async def place_order(
        ...
        """
        Place a trading order via Kalshi V2 order API.

        Translates the legacy (side=yes/no, action=buy/sell, price in cents)
        interface into the V2 format (side=bid/ask, price in dollar strings).
        ...
        Returns:
            Order response
        """
        # --- MASTER TRADING HALT (fail-safe kill switch) ---
        # Single universal chokepoint: every order from every bot/strategy passes
        # through here. When TRADING_HALTED is on, refuse to OPEN new positions
        # (any "buy"), regardless of how the bot was launched (--live flag,
        # LIVE_TRADING_ENABLED env, auto_trader, market making, etc.). Sells are
        # always permitted so existing positions can still be closed/exited.
        # Defaults to HALTED -- you must explicitly set TRADING_HALTED=false to
        # allow opening new positions again.
        if action == "buy" and os.getenv("TRADING_HALTED", "true").strip().lower() in ("1", "true", "yes", "on"):
            self.logger.warning(
                f"TRADING HALTED: blocked new position ({side} buy x{count} {ticker}). "
                "Set TRADING_HALTED=false to re-enable opening positions."
            )
            raise KalshiAPIError(
                "Trading halted by TRADING_HALTED kill switch -- new buy orders are blocked. "
                "Existing positions can still be sold/closed."
            )
        # --- End master trading halt ---

        if action == "buy" and side == "yes":
            v2_side = "bid"
            price_dollars = f"{yes_price / 100:.4f}" if yes_price is not None else None
        elif action == "sell" and side == "yes":
            v2_side = "ask"
            price_dollars = f"{yes_price / 100:.4f}" if yes_price is not None else None
        elif action == "buy" and side == "no":
            v2_side = "ask"
            price_dollars = f"{(100 - no_price) / 100:.4f}" if no_price is not None else None
        elif action == "sell" and side == "no":
            v2_side = "bid"
            price_dollars = f"{(100 - no_price) / 100:.4f}" if no_price is not None else None
        else:
            raise ValueError(f"Invalid side/action combo: {side}/{action}")
        ...
        if price_dollars is not None:
            order_data["price"] = price_dollars

        return await self._make_authenticated_request(
            "POST", "/trade-api/v2/portfolio/events/orders", json_data=order_data
        )
