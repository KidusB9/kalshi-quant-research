...
def dampen_confidence(
    ...
    """Shrink AI probability toward market-implied probability.

    LLMs are systematically overconfident. This applies Bayesian shrinkage:
    the final estimate is a weighted blend of the AI's prediction and the
    market price. A dampening_factor of 0.5 means we trust the AI and the
    market equally.

    Args:
        ai_probability: Raw LLM probability estimate, in [0, 1].
        market_probability: Market-implied probability (YES price), in [0, 1].
        dampening_factor: Weight given to market price. 0.0 = trust AI fully,
            1.0 = ignore AI and use market price.

    Returns:
        Blended probability estimate, clamped to [0.01, 0.99].
    """
    dampening_factor = max(0.0, min(1.0, dampening_factor))
    blended = (1.0 - dampening_factor) * ai_probability + dampening_factor * market_probability
