...
from src.utils.fees import round_trip_cost
...
class EdgeFilter:
    ...
    # TIGHTENED: Small account needs higher-quality trades only
    MIN_EDGE_REQUIREMENT = 0.08        # 8% minimum edge — don't trade thin edges
    HIGH_CONFIDENCE_EDGE = 0.06        # 6% edge for high confidence
    MEDIUM_CONFIDENCE_EDGE = 0.08      # 8% edge for medium confidence
    LOW_CONFIDENCE_EDGE = 0.12         # 12% edge for low confidence

    # TIGHTENED: Only trade when the model is actually confident
    MIN_CONFIDENCE_FOR_TRADE = 0.50    # 50% minimum confidence
    MAX_ACCEPTABLE_RISK = 0.6          # INCREASED: 60% max position risk (was 50%)

    # Minimum per-contract edge (in $) that must survive round-trip fees before
    # a trade is allowed. 2c gives a real cushion over Kalshi fees + slippage.
    MIN_NET_EDGE_AFTER_FEES = 0.02
    ...
    MAX_SPREAD_PCT_OF_MIDPOINT = 0.15  # Reject if spread > 15% of midpoint price
    ...
    def should_trade_market(
        ...
        # --- FEE-AWARE NET-EDGE GATE ---
        # On Kalshi the probability edge equals the per-contract dollar EV
        # (pay price p, receive $1 on win -> EV = q - p). A trade is only worth
        # taking if that edge survives the round-trip fee at the side we'd buy.
        # This kills near-certain churn (e.g. 91c repeats) and thin edges whose
        # "profit" is smaller than the fees -- the slow bleed in the logs.
        side = edge_result.side
        entry_price = market_probability if side == "YES" else (1.0 - market_probability)
        entry_price = max(0.01, min(0.99, entry_price))
        category = (additional_filters or {}).get("category", "standard")
        # Conservative: assume we may have to exit (taker both legs) at entry price.
        fee_per_contract = round_trip_cost(
            entry_price, entry_price, quantity=1,
            entry_type="taker", exit_type="taker", category=category,
        )
        net_edge = edge_result.edge_percentage - fee_per_contract
        if net_edge <= cls.MIN_NET_EDGE_AFTER_FEES:
            reason = (
                f"Net edge after fees too small: {edge_result.edge_percentage:.1%} edge "
                f"- ${fee_per_contract:.4f} fee/contract @ {entry_price:.2f} = "
                f"${net_edge:.4f} (need > ${cls.MIN_NET_EDGE_AFTER_FEES:.4f})"
            )
            return False, reason, edge_result
        # --- END FEE-AWARE GATE ---
        ...
        if additional_filters:
            ...
            bid_ask_spread = additional_filters.get('bid_ask_spread')
            midpoint_price = additional_filters.get('midpoint_price')
            if bid_ask_spread is not None and midpoint_price and midpoint_price > 0:
                spread_pct = bid_ask_spread / midpoint_price
                if spread_pct > cls.MAX_SPREAD_PCT_OF_MIDPOINT:
                    return False, f"Spread {bid_ask_spread:.2f} is {spread_pct:.0%} of midpoint {midpoint_price:.2f} (max {cls.MAX_SPREAD_PCT_OF_MIDPOINT:.0%})", edge_result
