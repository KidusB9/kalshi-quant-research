...
class EnsembleConfig:
    ...
    models: Dict[str, Dict] = field(default_factory=lambda: {
        "openai/gpt-4.1": {"provider": "openrouter", "role": "lead_analyst", "weight": 0.30},
...
class TradingConfig:
    ...
    min_balance: float = 5.0            # Low minimum for small accounts
    ...
    primary_model: str = "gemini-2.5-flash"  # Direct Gemini API (free, fast, no OpenRouter)
    fallback_model: str = "deepseek/deepseek-r1"  # OpenRouter fallback if Gemini unavailable
    ai_temperature: float = 0  # Lower temperature for more consistent JSON output
    ai_max_tokens: int = 8000    # Reasonable limit for reasoning models
