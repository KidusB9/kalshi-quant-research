...
async def execute_position(
    ...
    if live_mode:
        ...
        try:
            ...
            # For limit orders, use the ask price based on which side we're buying
            ...
            # For limit orders, we use the ask price (what we're willing to pay)
            ...
            if not is_tradeable_market(market):
                logger.warning(
                    f"  Skipping {position.market_id}: collection/aggregate ticker "
            ...
            if ask_cents <= 0 or ask_cents >= 100:
                logger.warning(
                    f"  Skipping {position.market_id}: {side_lower} ask price "
            ...
            if order_cost_cents > available_cents:
                logger.warning(
                    f"  Insufficient balance for {position.market_id}: order needs "
            ...
            # For a limit order, the fill price is the limit price or better.
...
async def place_sell_limit_order(
    ...
    try:
        ...
        if response:
            order_id = response.get('order_id') or response.get('order', {}).get('order_id') or client_order_id
    ...
    try:
        ...
        for position in positions:
            try:
                ...
                # Get current price based on position side (V2-safe)
                yb, ya, nb, na = get_market_prices(market_data)
                if position.side == "YES":
                    current_price = (yb + ya) / 2 if (yb + ya) > 0 else 0
                else:
                    current_price = (nb + na) / 2 if (nb + na) > 0 else 0
...
async def place_stop_loss_orders(
    ...
    try:
            ...
            try:
                ...
                if current_price > 0:
                    ...
                    if loss_pct <= stop_loss_threshold:  # Negative loss percentage
                        ...
                        logger.info(f" STOP LOSS TRIGGERED: {position.market_id} - {loss_pct:.1%} loss (${unrealized_pnl:.2f})")
        ...
        logger.info(f" Stop-loss summary: {results['orders_placed']} orders placed from {results['positions_processed']} positions")
        return results
